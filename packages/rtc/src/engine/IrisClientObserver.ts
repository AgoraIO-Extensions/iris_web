...
export class IrisClientObserver {
  ...
  async publishTrack(trackPackage: any) {
    let publishTrack: ILocalTrack;
    let globalVariables = this._engine.globalVariables;
    this._engine.entitiesContainer.irisClientList.map(
      async (irisClient: IrisClient) => {
        ...
        if (irisClient.agoraRTCClient?.channelName) {
          for (
            let i = 0;
            i < irisClient.agoraRTCClient.localTracks.length;
            i++
          ) {
  ...
  async unpublishTrack(trackPackage: any) {
    this._engine.entitiesContainer.irisClientList.map(
      async (irisClient: IrisClient) => {
        try {
          if (irisClient.agoraRTCClient?.channelName) {
  ...
  notify(type: NotifyType, scopePackage: any[]) {
    scopePackage.forEach(async (item) => {
      switch (type) {
        case NotifyType.PUBLISH:
          await this.publishTrack(item);
          break;
        case NotifyType.UNPUBLISH:
          await this.unpublishTrack(item);
          break;
      }
    });
  }

...
//存放一堆东西的
export class IrisEntitiesContainer {
  ...
  private _localAudioTracks: Array<AudioTrackPackage> = new Array<
    AudioTrackPackage
  >();
  private _localVideoTracks: Array<VideoTrackPackage> = new Array<
    VideoTrackPackage
  >();
  ...
  //subClient
  private _subClients: Container<IAgoraRTCClient> = new Container<
    IAgoraRTCClient
  >();
  private _subClientEventHandlers: Container<
    IrisClientEventHandler
  > = new Container<IrisClientEventHandler>();
  private _subClientAudioTracks: Container<
    Array<AudioTrackPackage>
  > = new Container<Array<AudioTrackPackage>>();
  private _subClientVideoTracks: Container<VideoTrackPackage> = new Container<
    VideoTrackPackage
  >();
  private _subClientTrackEventHandlers: Container<
    Array<IrisTrackEventHandler>
  > = new Container<Array<IrisTrackEventHandler>>();
  ...
  getSubClientRemoteUserByUid(uid: UID, connection: NATIVE_RTC.RtcConnection) {
  ...
  removeRemoteUserAndClearTrackEvent(
    ...
    this._mainClientTrackEventHandlers.filter(
      (trackEvent: IrisTrackEventHandler) => {
    ...
    subClientTrackEventHandlers &&
      subClientTrackEventHandlers.filter(
        (trackEvent: IrisTrackEventHandler) => {
          if (trackEvent.getRemoteUser() == user) {
            trackEvent.destruction();
            return false;
  ...
  walkAllILocalVideoTrack(fun: WalkILocalVideoPackageTrackFun) {
    if (this._mainClientLocalVideoTrack) fun(this._mainClientLocalVideoTrack);

    this._subClientVideoTracks.walkT(
      (channelId: string, uid: UID, trackPack: VideoTrackPackage) => {
        fun(trackPack);
      }
    );
  ...
  addSubClientLocalAudioTrack(
    connection: NATIVE_RTC.RtcConnection,
    trackPackage: AudioTrackPackage
  ) {
  ...
  removeSubClientLocalAudioTrack(
    connection: NATIVE_RTC.RtcConnection,
  ...
  setSubClientLocalVideoTrack(
    connection: NATIVE_RTC.RtcConnection,
    trackPack: VideoTrackPackage
  ) {
  ...
  clearSubClientLocalVideoTrack(connection: NATIVE_RTC.RtcConnection) {
  ...
  public getVideoFrameByConfig(
    ...
    if (uid == 0) {
      if (
        this._mainClientLocalVideoTrack &&
        this._mainClientLocalVideoTrack.type == type
      ) {
        return {
          video_track: this._mainClientLocalVideoTrack.track,
          is_new_frame: true, //todo  how to know is a new frame
  ...
  removeMainClientTrackEventHandlerByRemoteUser(
    user: IAgoraRTCRemoteUser,
    mediaType: 'audio' | 'video' | 'all'
  ) {
    this._mainClientTrackEventHandlers = this._mainClientTrackEventHandlers.filter(
      (trackEventHander: IrisTrackEventHandler) => {
        if (trackEventHander.getRemoteUser() != user) return true;
        ...
        if (
          mediaType == 'audio' &&
          trackEventHander.getTrackType() == 'IRemoteTrack'
        ) {
          trackEventHander.destruction();
          return false;
        }
        ...
        if (
          mediaType == 'video' &&
          trackEventHander.getTrackType() == 'IRemoteVideoTrack'
        ) {
          trackEventHander.destruction();
          return false;
        }
  ...
  addSubClientTrackEventHandler(
    connection: NATIVE_RTC.RtcConnection,
    eventHandler: IrisTrackEventHandler
  ) {
  ...
  removeSubClientTrackEventHandlerByTrack(
    connection: NATIVE_RTC.RtcConnection,
    track: ITrack
  ) {
    let array = this._subClientTrackEventHandlers.getT(
  ...
  removeSubClientTrackEventHandlerByRemoteUser(
    connection: NATIVE_RTC.RtcConnection,
    user: IAgoraRTCRemoteUser,
    mediaType: 'audio' | 'video' | 'all'
  ) {
  ...
  addSubClient(connection: NATIVE_RTC.RtcConnection, client: IAgoraRTCClient) {
    this._subClients.addT(connection.channelId, connection.localUid, client);
  }

  addSubClientEventHandler(
    connection: NATIVE_RTC.RtcConnection,
  ...
  clearSubClientAll(connection: NATIVE_RTC.RtcConnection) {
  ...
  getSubClient(connection: NATIVE_RTC.RtcConnection): IAgoraRTCClient {
    return this._subClients.getT(connection.channelId, connection.localUid);
  }

  getSubClients(): Container<IAgoraRTCClient> {
    return this._subClients;
  }

  getSubClientVideoTrack(
    connection: NATIVE_RTC.RtcConnection
  ): VideoTrackPackage {
  ...
  async audioTrackWillClose(audioTrack: ILocalAudioTrack) {
    ...
    if (
      this._mainClient &&
      this._mainClient.localTracks.indexOf(audioTrack) != -1
    ) {
      try {
    ...
    this._subClientAudioTracks.walkT(
      (channelId: string, uid: UID, packages: AudioTrackPackage[]) => {
    ...
    this._subClientTrackEventHandlers.walkT(
      (channelId: string, uid: UID, events: IrisTrackEventHandler[]) => {
  ...
  async videoTrackWillClose(videoTrack: ILocalVideoTrack) {
    ...
    if (
      this._mainClient &&
      this._mainClient.localTracks.indexOf(videoTrack) != -1
    ) {
      try {
    ...
    this._subClientVideoTracks.walkT(
      (channelId: string, uid: UID, pack: VideoTrackPackage) => {
  ...
  async destruction() {
    ...
    this._subClientEventHandlers.walkT(
      (channelId: string, uid: UID, t: IrisClientEventHandler) => {
        t.destruction();
      }
    );
    this._subClientEventHandlers = new Container();
    this._subClientTrackEventHandlers.walkT(
      (channelId: string, uid: UID, t: Array<IrisTrackEventHandler>) => {
    ...
    // this._remoteUsers = new Container();
  }
}
